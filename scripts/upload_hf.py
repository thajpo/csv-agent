...
def dataset_stats(dataset: DatasetDict) -> dict:
    stats: dict = {"splits": {}, "total": 0, "difficulties": {}, "csv_sources": set()}
...
def main():
    parser = argparse.ArgumentParser(description="Upload csv-agent episodes to Hugging Face Hub")
    parser.add_argument(
        "--repo-id",
        "--repo",
        dest="repo_id",
    ...
    if args.include_csvs:
        ...
        print(f"CSV root: {args.csv_root} ({sum(1 for p in args.csv_root.rglob('*') if p.is_file())} files)")
