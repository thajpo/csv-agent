...
from csv_spec import extract_python_cells
...
def parse_execution_result(output: str) -> CodeCellResult:
    ...
    if "stderr:\n" in output:
        ...
        return CodeCellResult(
            success=True, stdout=output_stdout, stderr=output_stderr, code=code
        )
...
def get_turn_validation_feedback(
    response: str, code_cells: list[str], min_reasoning_length: int = 10
) -> str:
    """
    ...
    if len(code_cells) == 0:
        return (
            "❌ No code block found. You must write exactly ONE ```python code block."
        )
