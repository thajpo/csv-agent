...
class EpisodeGenUI:
    ...
    Focus Mode: When running concurrent containers, only one "focused" container
    shows detailed output. Others are silent. This prevents output chaos.
    """

    # Class-level focus tracking (shared across all instances)
    ...
    def __init__(self):
        ...
        self._my_focus_id: str | None = None

    def acquire_focus(self, focus_id: str) -> bool:
        """Try to become the focused container. Returns True if acquired."""
        with self._focus_lock:
            if EpisodeGenUI._focused_id is None:
                EpisodeGenUI._focused_id = focus_id
                self._my_focus_id = focus_id
                return True
            return False

    def release_focus(self) -> None:
        """Release focus so another container can take it."""
        with self._focus_lock:
            if EpisodeGenUI._focused_id == self._my_focus_id:
                EpisodeGenUI._focused_id = None
            self._my_focus_id = None

    def has_focus(self) -> bool:
        """Check if this instance has focus."""
        return (
            self._my_focus_id is not None
            and self._my_focus_id == EpisodeGenUI._focused_id
        )

    @classmethod
    def reset_focus(cls) -> None:
        """Reset focus (call at start of pipeline)."""
        with cls._focus_lock:
            cls._focused_id = None
    ...
    def print_pipeline_header(
        ...
        self.base.print_key_value(
            "  Questions", f"{n_questions} questions loaded", indent=2
        )
        self.base.print_key_value("  Model", model, indent=2)
        self.base.print_key_value(
            "  N Consistency", f"{n_consistency} runs per question", indent=2
        )
        self.base.print_key_value("  Float Tolerance", f"±{float_tol}", indent=2)
        self.base.print_key_value(
            "  Total LLM Calls",
            f"{total_calls} ({n_questions} × {n_consistency + 1})",
            indent=2,
        )
    ...
    def print_question_header(self, q_num: int, total: int, question: dict) -> None:
        """Print question section header (only if focused)."""
        ...
        self.console.print(
            f"[bold magenta]│ QUESTION {q_num}/{total} - {difficulty} ({n_steps} steps)"
            + " "
            * (58 - len(f" QUESTION {q_num}/{total} - {difficulty} ({n_steps} steps)"))
            + "│[/bold magenta]"
        )
        ...
        for line in wrapped_q.split("\n"):
            padding = 56 - len(line)
            self.console.print(
                f"[bold magenta]│[/bold magenta] {line}"
                + " " * padding
        ...
        if hint:
            self.console.print("[bold magenta]│" + " " * 58 + "│[/bold magenta]")
            self.console.print(
                "[bold magenta]│[/bold magenta] [dim]Hint:[/dim]"
                + " " * 51
                + "[bold magenta]│[/bold magenta]"
            )
            # Truncate long hints
    ...
    def print_trace_header(self, mode: str, hint: str | None = None) -> None:
        """Print trace execution header (only if focused)."""
        if not self.has_focus():
            return
    ...
    def print_trace_start(self, mode: str) -> None:
        """Print trace start indicator (only if focused)."""
        if not self.has_focus():
            return
    ...
    def print_api_call_start(self, turn_num: int, max_turns: int) -> None:
        """Print API call start indicator (only if focused)."""
        if not self.has_focus():
            return
        self.console.print(
            f"[dim]    → Turn {turn_num}/{max_turns}: Calling LLM API...[/dim]"
        )

    def print_api_call_complete(self, turn_num: int, elapsed_seconds: float) -> None:
        """Print API call completion with timing (only if focused)."""
        if not self.has_focus():
            return
        self.console.print(
            f"[dim]    ✓ Turn {turn_num} response received ({elapsed_seconds:.1f}s)[/dim]"
        )
    ...
    def print_turn(
        ...
        """Print a single turn with LLM response and execution (only if focused)."""
        if not self.has_focus():
            return
        ...
        for i, (code, result) in enumerate(zip(code_cells, execution_results), 1):
            ...
            code_display = (
                code if len(code) <= 500 else code[:500] + "\n# ... (truncated)"
            )
            ...
            if result["success"]:
                ...
                if len(display_stdout) > 200:
                    display_stdout = display_stdout[:200] + "..."
                if display_stdout:
                ...
                if stderr.strip():
                    ...
                    err_display = stderr.strip()
                    if len(err_display) > 200:
                        err_display = err_display[:200] + "..."
                    self.console.print(f"[red]  Error:[/red] {err_display}")
    ...
    def print_trace_complete(
        ...
        """Print trace completion status (only if focused)."""
        if not self.has_focus():
            return

        answer_display = truncate_value(final_answer)
        if success:
            self.base.print_success(f"Answer submitted: {answer_display}")
            if elapsed_seconds is not None:
                self.console.print(
                    f"[dim]  Completed in {turns} turn(s) • {elapsed_seconds:.1f}s total[/dim]"
                )
            else:
    ...
    def print_triangulation_result(
        self, gold_trace, consistency_traces: list, verified: bool, float_tol: float
    ) -> None:
        """Print triangulation summary with verification status (only if focused)."""
        if not self.has_focus():
            return
        ...
        gold_answer = (
            ...
            if isinstance(gold_trace, dict)
            else gold_trace.final_answer
        )
        gold_hash = (
            gold_trace.get("final_answer_hash")
            if isinstance(gold_trace, dict)
            else gold_trace.final_answer_hash
        )
        self.console.print(
            f"[bold cyan]│[/bold cyan]   Answer: {truncate_value(gold_answer)}"
        )
        self.console.print(
            f"[bold cyan]│[/bold cyan]   Hash:   {gold_hash[:16]}..."
            if gold_hash
            else "[bold cyan]│[/bold cyan]   Hash:   None"
        )
        ...
        for i, trace in enumerate(consistency_traces, 1):
            if trace is None:
                self.console.print(
                    f"[bold cyan]│[/bold cyan]   Trace {i}: [red]FAILED[/red]"
                )
                continue
            ...
            answer_hash = (
                trace.get("final_answer_hash")
                if isinstance(trace, dict)
                else trace.final_answer_hash
            )

            matches = answers_match(
                gold_hash, answer_hash, gold_answer, answer, float_tol=float_tol
            )
            check = "✓" if matches else " "
            hash_display = (
                f"{answer_hash[:8]}..." if isinstance(answer_hash, str) else "None"
            )
            self.console.print(
                f"[bold cyan]│[/bold cyan]   Trace {i}: {truncate_value(answer, 40)}  ({hash_display}) {check}"
            )

        valid_answers = [
            (t.get("final_answer") if isinstance(t, dict) else t.final_answer)
            for t in consistency_traces
            if t is not None
            and (t.get("final_answer") if isinstance(t, dict) else t.final_answer)
            is not None
        ]
        ...
        self.console.print("[bold cyan]│[/bold cyan]")
        self.console.print(
            f"[bold cyan]│[/bold cyan] [bold]Majority:[/bold] {truncate_value(majority_value)} ({majority_count}/{len(consistency_traces)} votes)"
        )

        self.console.print("[bold cyan]│[/bold cyan]")
        if verified:
            self.console.print(
                "[bold cyan]│[/bold cyan] [bold green]✓ VERIFIED[/bold green]"
            )
        else:
            self.console.print("[bold cyan]│[/bold cyan] [bold red]✗ FAILED[/bold red]")

        self.console.print("[bold cyan]│[/bold cyan]")
        self.console.print("[bold cyan]" + "└" + "─" * 58 + "┘[/bold cyan]")

    def print_progress_summary(
        self, current: int, total: int, verified_count: int
    ) -> None:
